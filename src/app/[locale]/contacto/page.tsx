...
export default function ContactoPage({ params }: Props) {
  ...
  const locale = isLocale(routeLocale) ? routeLocale : "es";
  const { contactoPage, ui } = getDictionary(locale);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!res.ok) {
        setStatus({
          kind: "error",
          message: data?.error || ui.contact.sendError,
        });
        return;
      }
      ...
      form.reset();
      setStatus({ kind: "success", message: ui.contact.success });
    } catch (error) {
      setStatus({
        kind: "warning",
        message:
          process.env.NODE_ENV === "development"
            ? `${ui.contact.networkWarning} (${
      ...
      <Reveal delayMs={70}>
        <section className="-mx-5 bg-[#efefef] px-5 pb-10 md:-mx-8 md:px-8">
          ...
          >
            ...
            >
              {isSending ? ui.contact.sending : contactoPage.submit}
            </button>
